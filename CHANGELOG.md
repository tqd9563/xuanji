...
### 变更
- **看板信息架构原型迭代(等审批)**:针对「等待输入/已完成堆积一屏放不下」——①看板锁定一个视口高,列头(状态+计数)常驻,卡片在列内独立滚动(细滚动条,折叠区卡片不参与键盘导航);②「已完成」按归档语义降级为单行紧凑卡(标题+项目芯片限宽+完成时间,细节留悬停与回放),默认只显示最近 5 条,更早的收进「更早的 N 条 ▾」折叠区;窄屏(≤960px)回退为整页滚动。mock 数据扩充到贴近真实规模(6 等待 + 14 完成)供审阅。
