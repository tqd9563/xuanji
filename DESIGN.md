...
- **指示器:** 标准 `.sb-item`:前置圆点 + 文案。内存读作「内存 72%」——实际占用率 =(active + wired + 压缩器占用页)÷ `hw.memsize`,可回收缓存与真空闲不算占用;**状态栏不出现任何 swap 数据**(swap 总量由系统按需扩缩,占比无意义)。CPU 读作「CPU 33%」,数字 mono `tabular-nums`。颜色只来自判定等级:正常 muted + 玉色圆点,警告整体 `{colors.amber}`(圆点复用 2s 脉冲),严重 `{colors.red}`;采样失败圆点变 1px 虚线空心、数字换成「—」,文字写明「采样失败」,**永不 color-alone**。内存等级直接取内核 `kern.memorystatus_vm_pressure_level`(1/2/4,与活动监视器同源),不提供阈值设置;CPU 用「用户 + 系统」对照可调阈值(默认黄 60 / 红 85)。两者都受「连续 N 次才变色」防抖。术语两边统一:压力正常 / 压力警告 / 压力严重。
...
- **条形:** 两条(物理内存、CPU)一律照抄仪表盘 token 用量「开发 vs multica」对比条 `.cmp-track`:10px 高、5px 圆角、段间无缝、底轨 `--tint-muted`(muted 12%)。**空闲/未用不画段,直接露出底轨**;图例用 `.cmp-legend`,「空闲」那枚方块与底轨同色。物理内存四段:应用占用(active+wired,`chart-1`)| 压缩器(`chart-2`)| 可回收缓存(inactive+purgeable+speculative,`chart-3`)| 真空闲(底轨)。物理内存行右侧标「占用 N%」,与状态栏同一个数。**Swap 不画条**:只一行「Swap(磁盘) · 已用 15.1G」,数字颜色跟随内存等级——swap 总量由 macOS 按需扩缩(实测 17.4G → 16.0G),画成「占总量比例」会误导。CPU 条用户 `chart-1` / 系统 `chart-2`。
- **排行:** 进程树按 ppid 聚合到应用(`.app` 包名归一;launchd 直属系统服务单列),可展开到进程。行是 4 列 grid(14px chevron | 名称 | 两列 64px 右对齐 mono 数值);内存 tab 按「总占用」(top MEM)降序,两列为「总占用 / 压缩+换出」(总占用在前,展开的进程同口径排序;建议规则仍按压缩+换出选对象),CPU tab 为「%CPU / 占整机」(%CPU 单核 100%、满载 = 核数 × 100,口径放列头 title,不另写说明段)。璇玑派发会话归入一组并打 `t-susp` 小标「璇玑」,终端 claude 会话归入另一组打中性「终端」标。
...
- **外观来源:** 默认「跟随 Ghostty」:主题(Catppuccin Frappe)、字体、字号、背景色/不透明度/模糊、光标样式与闪烁全部读本机 Ghostty 配置并解析其主题文件;字体按 **Ghostty 实际渲染的**来,不按配置写的名字(配置写 Maple Mono NF CN 但本机未装,Ghostty 回退到内置 JetBrains Mono,璇玑随之用 JetBrains Mono)。光标闪烁**只听设置**:吞掉程序发来的 DECSET/DECRST 12 并在 DECSCUSR 后重设闪烁(xterm-256color 的 cnorm 自带 `?12l`,不吞则一启动就停闪),形状仍交给程序。任一项在设置里被改动即整体转「自定义」,一键可回到跟随。读不到 Ghostty 时回退「璇玑 · 玉」主题 + 100% 不透明 + 模糊 0。
...
- **设置 › 终端:** 各行**只有标签与控件,不写说明小字**(用户 2026-09-24 明确要求删掉);唯一例外是「呼出 / 收起」检测到上游占用时出一行 amber 提示。顶部一条来源说明(绿点 + 「已读取 Ghostty x.y 配置 <路径>」+ 重新读取)。分组「外观」(外观来源 · 主题卡片 5 列)、「背景」(背景颜色取色器 + 跟随配色 / 背景不透明度 30–100 / 背景模糊 0–40 / 背景图片 / 图片不透明度)、「文字与光标」(字体只读展示 / 字号 11–20 / 光标竖线·方块·下划线 + 闪烁)——均「本机」;「行为」(呼出快捷键 / 新终端默认目录 / 切换视图时)为「账户」。
