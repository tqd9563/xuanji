...
### 变更
- **用量模块头部写明条长口径**：Token 单位下的小字由「条长 = token 量」改为「条长未包含 cacheRead」，读条形图时即可知道 cacheRead 不在其中；完整口径在悬停里。
- **仪表盘下半部布局**：左栏改为「最近活动」并拉通整栏（12 条 → 40 条，可视区封顶 60vh 后内部滚动），右栏是合并后的「Token 用量」模块（对比条 + 按项目条形图 + 7 日热力图）。原先独立占据一整行的「Token 成本」面板已并入该模块。

### 修复
- **切换成本/Token 时两组切换按钮错行**：头部是 `flex-wrap` 容器，而 flex 的分行规则是「先换行再压缩」，小字按内容宽度参与分行，口径文案一变长就把第二组切换顶到下一行、模块头忽高忽低。改为小字 `flex: 1 1 0` 吸收剩余空间（顺带接替 spacer），它不再参与分行决策，两种单位在任何窗口宽度下头部高度一致；空间不足时小字省略号收尾。
